...
用户会提供一个机场网站地址，可能附带账号密码。也可能同时提供一个 HTTP 请求抓包文件（CSV 格式，如 Proxyman 导出），格式如：
```
/explore-airport-api https://example.com/#/invite email password 对应http请求文件：/path/to/file.csv
```

## 执行步骤

### 第零步：如果有 CSV 抓包文件，优先分析（捷径）

如果用户提供了 HTTP 请求抓包文件（CSV），这是最快路径，**跳过第二步和第三步**：

1. 用 Read 工具读取 CSV 文件
2. 从 CSV 中提取关键信息：
   - **Host/Path 列**：直接得到 `base_url` 和 `api_prefix`（如路径都是 `/api/v1/...` 开头，说明 api_prefix = `/api/v1`）
   - **Method 列**：知道每个接口是 GET 还是 POST
   - **Remote Address 列**：判断是否经过代理（如 `::1:7890` 说明用了代理）
3. 筛选出目标域名（排除无关请求如 client.crisp.chat 等第三方域名）的请求，列出所有 API 端点
4. 从端点路径直接判断面板类型（V2Board、SSPanel 等）
5. 如果现有面板类型匹配（如标准 V2Board），**直接复用，不创建新面板**
6. 跳到第四步（登录测试）

### 第一步：识别面板类型（无 CSV 时执行）
...
1. **有 CSV 抓包文件就用 CSV** → 比任何探索都快，直接拿到所有 API 端点和路径
2. **先用 cmux 登录一次抓真实请求** → 拿到 api_prefix 和认证格式（无 CSV 时）
3. **然后用脚本批量探测接口** → 比浏览器一个个点快 10 倍
4. **数据对不上就换接口** → 不要死磕一个 API，数据大概率在别的端点
5. **参考已有面板实现** → 看 `panels/v2board.py` 的模式
6. **探索结果必须完整转化到代码** → 探索完成后，逐条对照发现编写面板代码，不能遗漏任何有数据的页面或接口。特别是邀请/佣金页面，往往包含邀请人数、累计佣金、可提现佣金等仪表盘上没有的数据。
